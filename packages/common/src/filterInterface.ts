...
export function getValuesForValuePath(value: any, path: ValuePath, index: number = 0): any[] {
  ...
  switch (pathSegment.type) {
    case ValuePathSegmentType.List: {
      ...
      return value.reduce((values: any[], value) =>
        [...values, ...getValuesForValuePath(value, path, index + 1) || []], []
      )
    }

    case ValuePathSegmentType.Map: {
      if (typeof value !== 'object') return []
      return Object.values(value).reduce((values: any[], value) =>
        [...values, ...getValuesForValuePath(value, path, index + 1) || []], []
      )
    }
...
export interface FullTextFilter {type: FilterType.FullText, value: string}
export function FullTextFilter(value: string): FullTextFilter {
  return {type: FilterType.FullText, value}
}

export interface CompositeFilter {type: FilterType.Composite, filters: Filter[]}
export function CompositeFilter(filters: Filter[]): CompositeFilter {
  return {type: FilterType.Composite, filters}
}

export interface ValueFilter {type: FilterType.Condition, condition: Condition}
