      ...
      switch (field.type) {
        case 'fieldset': {
          path.push({
            ...
            key: lastItem(nextField.keyPath)!.toString()
          })
          break
        }
      }
    ...
    case 'text':
      return {
        ...
        path: [
          ...
          ...objectPathForField(field, fields)
        ],
...
export function sortConfigurationsForViewContext(viewContext: ViewContext): SortConfigration[] {
  ...
  const descriptionFields = (viewContext.descriptionKeyPaths || []).map(keyPath =>
    findKeyPath(keyPath, fields)
  )
    ...
    case 'fieldset': {
      ...
      childFields.push(
        ...children.reduce(
          (prev, childField) => {
            ...
            const pathSegment =
              field.type === 'fieldset' ? StructPathSegment(key) : UnionPathSegment(key)
      ...
      if (field.type === 'select') {
        ...
        conditionGroups.push({
          id: shortid.generate(),
          label: 'Union Key',
          conditions: [{id: shortid.generate(), type: ConditionType.UnionKeyEqual, path, options}]
        })
      }
...
export function filterConfigurationsForViewContext(viewContext: ViewContext): FilterFieldGroup[] {
  ...
  return [
    ...
    },
    {
      id: shortid.generate(),
      label: 'Field',
      fields: filterConfigurationsForField(rootField, viewContext.fields, [
        {type: ValuePathSegmentType.Struct, key: 'value'}
      ])
    }
  ]
