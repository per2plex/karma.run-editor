...
const sharedRequestClient = axios.create({
  baseURL: Env.KARMA_MEDIA_SERVER_URL || '',
  timeout: 30000
})
...
export async function commitMedia(
  ...
  const response = await sharedRequestClient.post(
    path,
    {
      tempID,
...
export async function copyMedia(
  ...
  const response = await sharedRequestClient.post(
    path,
    {url, filename, folder: opts.folder},
    {
      headers: {'X-Key': opts.apiKey}
    }
  )
...
export async function deleteMedia(type: string, id: string, opts: CommonOptions): Promise<void> {
  const path = `/api/${opts.name}/delete`
  await sharedRequestClient.post(
    path,
    {type, id},
    {
      headers: {'X-Key': opts.apiKey}
