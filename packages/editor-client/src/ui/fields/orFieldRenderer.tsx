...
export class OrFieldRenderer extends React.Component<OrFieldRenderer.Props> {
  ...
  public render() {
    ...
    const notEmptyFieldset = !(
      selectedStore instanceof FieldsetStore && Object.keys(selectedStore.fields).length === 0
    )
    ...
    return (
      <FieldWrapper depth={this.props.depth} index={this.props.index}>
        <Field depth={this.props.depth} index={this.props.index}>
          ...
          <Select
            value={String(this.props.store.selectedIndex)}
            options={options}
            onChange={this.handleChange}
            disabled={this.props.disabled}
          />
        </Field>
        <FieldInset>{content}</FieldInset>
      </FieldWrapper>
    )
  }
}
