...
export class LinearFieldsetRenderer extends React.Component<LinearFieldsetRenderer.Props> {
  public render() {
    ...
    const fields = Object.entries(this.props.store.fields).map(([key, store], index) =>
      renderValueStore(store, {
        ...options,
        key,
        index,
...
export class TabFieldsetRenderer extends React.Component<
  TabFieldsetRenderer.Props,
  TabFieldsetRenderer.State
> {
  ...
  public render() {
    let depth = this.props.isWrapped ? this.props.depth : this.props.depth - 1

    const {children, store, ...options} = this.props
    const field = renderValueStore(store.fields[this.state.selectedKey], {
      ...options,
      depth: depth + 1,
      isWrapped: true
    })
    ...
    return (
      <FieldWrapper depth={this.props.depth} index={this.props.index}>
        <Field
          className={TabFieldsetRenderer.Style}
          depth={this.props.depth}
