...
export interface InputMessage<T, I> {
  id: string, type: T, input: I
}

export interface OutputMessage<T, O> {
  id: string, type: T, output: O
}
...
export type MessageTypeMap<T extends MessageMap> = {
...
export function createWorkerInterface<T extends MessageMap>(
  worker: Worker, messageTypeMap: MessageTypeMap<T>
  ...
  return mapObject(messageTypeMap, (_value, type) =>
    (input: any) => new Promise<any>(
      (resolve) => {
