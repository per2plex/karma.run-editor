...
export const OptionalModifier = BaseModifierConstructor<'optional', OptionalModifierOptions>('optional')

// ListModifier
export interface ListModifierOptions {minLength?: number, maxLength?: number}
...
export const RecursionModifier = BaseModifierConstructor<'recursion', RecursionModifierOptions>('recursion')
...
// ErrorField
export interface ErrorFieldOptions {message?: string}
...
// CheckboxField
export interface CheckboxFieldOptions {defaultValue?: boolean}
...
// DateTimeField
export interface DateTimeFieldOptions {defaultValue?: Date, format?: DateFormat}
...
// EnumField
export interface EnumFieldOptions {values?: Select.Option[]}
...
// Fieldset
export interface FieldsetOptions {layout?: Layout}
...
export const KarmaMediaField = BaseFieldConstructor<'karmaMedia', KarmaMediaFieldOptions>('karmaMedia')

// PasswordField
export interface PasswordFieldOptions {costFactor?: number}
...
// RefField
export interface RefFieldOptions {model?: string, disableEditing?: boolean}
...
// SelectField
export interface SelectFieldOptions {defaultValue?: string}
...
// Field
export type Field =
  Fieldset | CheckboxField | DateTimeField
  | EnumField | FloatField | TupleField | OrField
  | IntField | KarmaMediaField | PasswordField
  | RefField | RichTextField | SelectField | TextField | ErrorField
  | RecursionField | NullField
  | UntypedField
...
export function deduceFieldsFromModel(
  model: Model, keyPath: KeyPath = [], modifiers: Modifier[] = [], typeHint?: FieldType
): Field[] {
  ...
  if (typeHint) {
    switch (typeHint) {
      case 'richText': {
        if (model.type === 'struct') {
          const blockData: Model | undefined = getValueForKeyPath(model, [
            'fields', 'blocks', 'model', 'fields', 'data'
          ])

          const entityData: Model | undefined = getValueForKeyPath(model, [
            'fields', 'entityMap', 'model', 'fields', 'data'
          ])
          ...
          if (blockData && blockData.type === 'or') {
            ...
            if (blockDataUnion && blockDataUnion.type === 'union') {
              blockDataFields = Object.entries(blockDataUnion.fields).reduce((prev, [key, model]) => (
          ...
          if (entityData && entityData.type === 'or') {
            ...
            if (entityDataUnion && entityDataUnion.type === 'union') {
              entityDataFields = Object.entries(entityDataUnion.fields).reduce((prev, [key, model]) => (
      ...
      case 'karmaMedia': return [KarmaMediaField(defaultOptions)]
    }
  }

  // Modifiers
  switch (model.type) {
    case 'optional': return deduceFieldsFromModel(
      model.model, keyPath, [...modifiers, OptionalModifier({})]
    )

    case 'list': case 'set': return deduceFieldsFromModel(
      model.model, keyPath, [...modifiers, ListModifier({})]
    )

    case 'map': return deduceFieldsFromModel(
      model.model, keyPath, [...modifiers, MapModifier({})]
    )

    case 'annotation': {
      const typeHint = model.value.replace('field:', '')
      return deduceFieldsFromModel(
        model.model, keyPath, modifiers, typeHint as FieldType
      )
    }

    case 'unique': return deduceFieldsFromModel(
      model.model, keyPath, modifiers
    )
  }
  ...
  switch (model.type) {
    case 'union': case 'struct': {
      const fields = Object.entries(model.fields).reduce((prev, [key, model]) => (
        prev.concat(deduceFieldsFromModel(model, [...keyPath, key]))
      ), [] as Field[])

      return [
        model.type === 'struct'
          ? Fieldset(defaultOptions)
    ...
    case 'or': case 'tuple': {
      const fields = model.fields.reduce((prev, model, index) => (
        prev.concat(deduceFieldsFromModel(model, [...keyPath, index]))
      ), [] as Field[])

      return [
        model.type === 'tuple'
          ? TupleField(defaultOptions)
          : OrField(defaultOptions),
        ...fields
      ]
    }

    case 'null': return [NullField(defaultOptions)]
    case 'bool': return [CheckboxField(defaultOptions)]
    case 'string': return [TextField(defaultOptions)]
    case 'dateTime': return [DateTimeField(defaultOptions)]
    case 'ref': return [RefField({...defaultOptions, model: model.model})]

    case 'enum': return [EnumField({
      ...defaultOptions, values: model.values.map(value => (
        {key: value, label: convertKeyToLabel(value)}
      ))
    })]

    case 'float': return [FloatField(defaultOptions)]

    case 'int': return [IntField(defaultOptions)]
    case 'int8': return [IntField({...defaultOptions, storageSize: 8})]
    case 'int16': return [IntField({...defaultOptions, storageSize: 16})]
    case 'int32': return [IntField({...defaultOptions, storageSize: 32})]
    case 'int64': return [IntField({...defaultOptions, storageSize: 64})]

    case 'uint': return [IntField({...defaultOptions, unsigned: true})]
    case 'uint8': return [IntField({...defaultOptions, storageSize: 8, unsigned: true})]
    case 'uint16': return [IntField({...defaultOptions, storageSize: 16, unsigned: true})]
    case 'uint32': return [IntField({...defaultOptions, storageSize: 32, unsigned: true})]
    case 'uint64': return [IntField({...defaultOptions, storageSize: 64, unsigned: true})]

    case 'recurse': return [RecursionField({...defaultOptions, recursionLabel: model.label})]

    case 'recursion': {
      const fields = deduceFieldsFromModel(model.model, [...keyPath, model.label])

      return [
        RecursionField({...defaultOptions, recursionLabel: model.label}),
        ...fields
      ]
    }
    ...
    case 'recursive': {
      ...
      const fields = modelEntries.reduce((prev, [key, model]) => (
        prev.concat(deduceFieldsFromModel(model, [...keyPath, key]))
      ), [] as Field[])

      return [
        RecursionField({...defaultOptions, recursionLabel: model.top}),
        ...fields
      ]
    }

    default: return [ErrorField({
      keyPath, label, modifiers, message: `Couldn't deduce field for type: ${model.type}`
    })]
  }
}

export function mergeViewContext(viewContextA: ViewContext, viewContextB: ViewContext): ViewContext {
...
export function mergeFields(fieldsA: Field[], fieldsB: Field[]): Field[] {
  const mergedFields = fieldsB.reduce((prev, fieldB) => {
