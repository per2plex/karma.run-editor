...
export const ViewContextTag = '_frontend_view_context_v1'
...
export const EnumFieldModel = m.Struct({
  values: m.List(m.Struct({
    key: m.String(), label: m.String()
  }))
})
...
export const RichTextFieldModel = m.Struct({
  controls: m.Optional(m.Set(m.Enum([
    'bold', 'italic', 'underline',
    'strikethrough', 'orderedList', 'unorderedList'
  ]))),
  links: m.Optional(m.List(m.Struct({
    type: m.Optional(m.String()), // TODO: Remove when migrated
    dataKey: m.String(),
    label: m.Optional(m.String())
  }))),
  styleGroups: m.Optional(m.List(m.Struct({
    label: m.Optional(m.String()),
    name: m.String(),
    styles: m.List(m.Struct({
      type: m.String(),
      style: m.Optional(m.Map(m.String())),
      label: m.Optional(m.String())
    }))
  }))),
  blocks: m.Optional(m.List(m.Struct({
    type: m.String(),
    dataKey: m.Optional(m.String()),
    style: m.Optional(m.Map(m.String())),
    label: m.Optional(m.String())
  }))),
  elements: m.Optional(m.List(m.Struct({
    type: m.String(),
    dataKey: m.Optional(m.String()),
    style: m.Optional(m.Map(m.String())),
...
export function unserializeViewContext(rawViewContext: any): ViewContext {
  ...
  return {...rawViewContext, fields: rawFields && rawFields.map(rawField => unserializeField(rawField))}
}
...
export function unserializeField(rawField: any): Field {
  ...
  // TODO: Validation
  const modifiers = rawModifiers && rawModifiers.map((rawModifier: any) => unserializeModifier(rawModifier))

  switch (type) {
    case undefined:
    case 'checkbox': case 'enum':
    case 'fieldset': case 'float': case 'int':
    case 'karmaMedia': case 'password': case 'ref': case 'richText':
    case 'select': case 'text': case 'tuple': return {...rawField, ...fieldValue, type, modifiers}

    case 'dateTime': return {
      ...rawField, ...fieldValue, type, modifiers, defaultValue: (
        fieldValue.defaultValue
          ? new Date(fieldValue.defaultValue)
          : undefined
      )
    }
    default: return ErrorField({
      ...rawField, message: 'Unknown field type!'
    })
  }
}
...
export function unserializeModifier(rawModifier: any): Modifier {
  ...
  switch (type) {
    case 'map': case 'list': case 'optional': return {...value, type}
