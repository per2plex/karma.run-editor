...
export interface BaseModel<T extends string> {type: T}
export interface RefModel<T extends string> extends BaseModel<T> {model: string}
export interface ContainerModel<T extends string> extends BaseModel<T> {model: Model}
export interface ArrayModel<T extends string> extends BaseModel<T> {fields: Model[]}
export interface FieldModel<T extends string> extends BaseModel<T> {fields: ModelMap}
export interface EnumModel<T extends string> extends BaseModel<T> {values: string[]}
export interface AnnotationModel<T extends string> extends BaseModel<T> {value: string, model: Model}

export interface RecurseModel<T extends string> extends BaseModel<T> {label: string}
export interface RecursionModel<T extends string> extends BaseModel<T> {label: string, model: Model}
export interface RecursiveModel<T extends string> extends BaseModel<T> {top: string, models: ModelMap}
...
export type Model =
  ...
  | Float | String | DateTime | Bool | Ref | Enum

  // Algebraic
  | List | Map | Set | Struct | Union | Tuple | Or

  // Semantic
  | Optional | Unique | Any | Annotation | Null
...
export function unserializeModel(rawModel: any): Model {
  ...
  switch (type) {
    ...
    case 'float': case 'string': case 'dateTime': case 'bool': case 'any': case 'null': {
    ...
    // ContainerModel
    case 'list': case 'map': case 'set': case 'optional': case 'unique': {
      if (typeof value !== 'object') {
        throw new Error(`Expected object, got ${typeof value}.`)
      }
    ...
    case 'struct': case 'union': {
      ...
      return FieldModelConstructor(type)(mapObject(value, model => unserializeModel(model))) as Model
    }
    ...
    case 'recursive': {
      ...
      return RecursiveModelConstructor(type)(
        value.top, mapObject(value.models, model => unserializeModel(model))
      ) as Model
    }
...
export function serializeModel(model: Model): any {
  ...
  switch (model.type) {
    ...
    // RefModel
    case 'ref':
      value = model.model; break

    // EnumModel
    case 'enum':
      value = model.values; break

    // ContainerModel
    case 'list': case 'map': case 'set': case 'optional': case 'unique': case 'annotation':
      value = serializeModel(model.model); break

    // ArrayModel
    case 'tuple': case 'or':
      value = model.fields.map(field => serializeModel(field)); break

    // FieldModel
    case 'struct': case 'union':
      value = mapObject(model.fields, (field) => serializeModel(field)); break

    // RecurseModel
    case 'recurse':
      value = model.label; break

    // RecursionModel
    case 'recursion':
      value = {label: model.label, model: serializeModel(model.model)}; break
    ...
    case 'unknown': return {[model.originalType]: model.value}
  }
