import {
  observable, action, reaction, runInAction,
  IObservableArray, ObservableMap, IReactionDisposer, computed
} from 'mobx'
...
import {
  ViewContextTag,
  ViewContext, createViewContextModel,
...
import {
  Session, authenticate, renewSession,
  Entry, TagTag, ModelTag,
  getAllEntriesForTag, getAllEntriesForModel, getEntryForID,
  createEntry, updateEntry, deleteEntry, APIError,
  serializeModel, unserializeModel, Model, UserTag, RoleTag
} from '../api/karma'

import {
  EditorContext, EditorContextID, EditorContextTag,
  createEditorContextModel, unserializeEditorContext
} from '../api/karmafe/editorContext'

import {
  ModelGroup, createModelGroupModel,
  ModelGroupTag, unserializeModelGroup
...
export async function createViewContextModelEntry(
  metaModelID: string, tagModelID: string, session: Session
) {
  ...
  const viewContextTag = {model: viewContextID, tag: ViewContextTag}
...
export async function createEditorContextModelEntry(
  metaModelID: string, tagModelID: string, modelGroupID: string, session: Session
) {
  ...
  const editorContextTag = {model: editorContextID, tag: EditorContextTag}
...
export class EditorStore {
  ...
  constructor() {
    ...
    reaction(() => this._activeEditorContext, async (activeEditorContext) => {
    ...
    reaction(() => this.isDeveloper, async (isDeveloper) => {
      if (isDeveloper) {
    ...
    // Session reaction
    reaction(() => this.session, async (session) => {
      if (session) {
  ...
  @action public async restoreSessionFromLocalStorage() {
    const rawSession = storage.get(SessionStorageKey)
    if (!rawSession) return
    return this.restoreSession(rawSession)
  }

  @action public async restoreSession(session: Session) {
    this.destroySession()
    ...
    try {
      ...
      runInAction('restoreSession', () => {this.session = newSession})
  ...
  /** @throws {APIError} */
  @action public async renewSession() {
    if (this.session) {
      const session = await renewSession(this.session)
      runInAction('renewSession', () => {this.session = session})
    }
  }

  /** @throws {APIError} */
  @action public async login(endpoint: string, database: string, username: string, password: string) {
    this.destroySession()

    const session = await authenticate(endpoint, database, username, password)
    await this.loadBaseData(session)
    runInAction('login', () => {this.session = session})
    this.startSessionRenewal()
    return session
  }

  @action public async loginWithSignature(endpoint: string, database: string, username: string, signature: string) {
    this.destroySession()
  ...
  @computed public get reverseTags() {
    return this.tags.entries().reduce((prev, [key, value]) => {
      prev[value] = key
      return prev
    }, {} as ObjectMap<string>)
  }

  @computed public get modelList() {
  ...
  /** @throws {APIError} */
  @action private async loadBaseData(session: Session) {
    ...
    let viewContextID = tags[ViewContextTag]
    ...
    this.disposeTagReaction = reaction(() => this.entries.get(tagModelID), tagEntries => {
      if (tagEntries) this.tags.replace(parseEntriesAsTagMap(tagEntries))
    }, true)

    this.disposeModelReaction = reaction(() => this.entries.get(metaModelID), modelEntries => {
    ...
    this.disposeViewContextReaction = reaction(() => {
      ...
      const defaultViewContextMap = createDefaultViewContextMap({
        userModelID, roleModelID, tagModelID,
        editorContextID: editorContextID!,
        modelGroupID: modelGroupID!,
        viewContextID: viewContextID!
      })

      this.viewContexts.push(...modelEntries.map(([id, model]) => {
        const tag = reverseTags[id]
        const deducedViewContext = deduceViewContextFromModel(id, model, tag)
        const matchingViewContexts = viewContexts.filter(viewContext => viewContext.model === id)
  ...
  /** @throws {APIError} */
  @action public async loadEntriesForModel(id: string) {
    if (!this.session) {
  ...
  /** @throws {APIError} */
  @action public async loadEntryForID(model: string, id: string) {
    if (!this.session) {
  ...
  /** @throws {APIError} */
  @action public async createEntry(model: string, value: any) {
    if (!this.session) {
  ...
  /** @throws {APIError} */
  @action public async updateEntry(model: string, id: string, value: any) {
     if (!this.session) {
  ...
  /** @throws {APIError} */
  @action public async deleteEntry(model: string, id: string) {
    if (!this.session) {
  ...
  /** @throws {APIError} */
  @action public async cloneEntry(model: string, id: string) {
  ...
  @computed public get viewContextMap() {
    return this.viewContexts.reduce((prev, viewContext) => {
      prev[viewContext.model] = viewContext
      return prev
    }, {} as ObjectMap<ViewContext | undefined>)
  }

  @computed public get activeEditorContext() {
  ...
  @computed public get activeEditorContextID() {
    if (!this._activeEditorContext) throw new Error('No active EditorContext set!')
  ...
  @computed public get activeModelGroups() {
    return this.activeEditorContext.modelGroups.map(groupID => {
      return this.modelGroups.find(modelGroup => modelGroup.id === groupID)!
    })
  }

  @action public toggleDeveloperMode() {
    this.isDeveloper = !this.isDeveloper
  }

  @computed private get fuzzyModelSearchInstance() {
    const modelIDs = this.activeModelGroups.reduce((modelIDs, group) => {
  ...
  @action public searchModel(term: string) {
    return this.fuzzyModelSearchInstance.search<{name: string, model: string, slug: string}>(term)
  }

  @action public increaseUnsavedChangesCount() {
    this.unsavedChangesCount++
  }

  @action public decreaseUnsavedChangesCount() {
    this.unsavedChangesCount--
  }
