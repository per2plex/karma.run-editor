...
function createModifierFieldStores(
  modifierIndex: number, field: Field, fields: Field[],
  ...
  switch (modifier.type) {
    ...
    default: return createBasicFieldStores(field, fields, recursions)
  }
}

function createBasicFieldStores(field: Field, fields: Field[], recursions: FieldStoreMap[]): FieldStore {
  const commonOptions = {
    label: field.label,
    description: field.description,
    icon: field.icon
  ...
  switch (field.type) {
    case 'checkbox': return new CheckboxFieldStore({
      ...commonOptions, value: field.defaultValue
    })
    ...
    case 'text': return new TextFieldStore({
      ...commonOptions,
    ...
    case 'int': return new IntFieldStore({...commonOptions, value: field.defaultValue})
    case 'float': return new FloatFieldStore({...commonOptions, value: field.defaultValue})

    case 'dateTime': return new DateTimeFieldStore({
      ...commonOptions,
      format: field.format,
      value: field.defaultValue
    })

    case 'password': return new PasswordFieldStore({
      ...commonOptions,
    ...
    case 'recursion': {
      const childFields = findChildrenOfKeyPath(field.keyPath, fields)
      const recursionMap = recursions.reduce((prev, recursionMap) => {
      ...
      for (const field of childFields) {
        ...
        if (recursionMap[key]) return new ErrorFieldStore({
          ...commonOptions, message: 'Duplicate recursion key!'
        })
      ...
      return new RecursionFieldStore(recursionsInScope, {
        ...commonOptions, recursionLabel: field.recursionLabel
      })
    }

    case 'null': return new NullFieldStore(commonOptions)
    case 'error': return new ErrorFieldStore({
      ...commonOptions, message: field.message
    })

    default: return new ErrorFieldStore({
      ...commonOptions, message: `Unknown Field Type: ${field.type}`
    })
  }
}

export function createFieldStoreForField(field: Field, fields: Field[], recursions: FieldStoreMap[]): FieldStore {
