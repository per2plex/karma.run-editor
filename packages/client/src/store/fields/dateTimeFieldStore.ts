...
export class DateTimeFieldStore implements FieldStore {
  ...
  @action public change(value: Date | string) {
    this.value = value
    this.validate()
  }
  ...
  public fits(data: any) {
    switch (this.format) {
      case 'rfc3339': return !isNaN(Date.parse(data))
  ...
  public async asJS() {
    if (typeof this.value === 'object') {
      switch (this.format) {
        case 'timestamp': return Math.round(this.value.getTime() / 1000)
        case 'rfc3339': return this.value.toISOString()
      }
    }
