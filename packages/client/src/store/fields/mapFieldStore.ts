...
export class MapFieldStore implements FieldStore {
  ...
  @action public fill(data: any) {
    ...
    if (this.restrictedToKeys) {
      ...
      values = this.restrictedToKeys.reduce((sortedValues, key) => {
  ...
  @action public changeKeyAt(index: number, key: string) {
    if (index < 0 || index > this.values.length) throw new Error('Invalid Index!')
    this.values[index].key = key
  }

  @action public insertValueAt(index: number, value: string) {
    if (index < 0 || index > this.values.length) throw new Error('Invalid Index!')
  ...
  public async asJS() {
    const entries = await Promise.all(this.values.map(async (value) => {
