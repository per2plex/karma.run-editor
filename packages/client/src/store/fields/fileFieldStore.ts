...
import {
  uploadMedia, getThumbnailURL,
  commitMedia, KarmaMediaCommitResponse,
  copyMedia, deleteMedia
} from '../../api/karmaMedia'
...
export class FileFieldStore implements FieldStore {
  ...
  @action private handleUploadProgress = (e: ProgressEvent) => {
    this.uploadProgress = Math.round(e.loaded / e.total)
  }

  @action public changeValue(value: FileValue | undefined) {
    this.value = value
    this.validate()
  }

  @action public changeTempValue(value: TempFileValue | undefined) {
  ...
  public fits(data: any) {
    ...
    switch (this.format) {
      case 'legacy': return isLegacyJSFileValue(data)
      case 'v1': return isJSFileValue(data)
    }
  }
    ...
    switch (this.fileHost.type) {
      case 'karmaMedia': {
        ...
        const result = await uploadMedia(file, {
          ...rest, onProgress: this.handleUploadProgress
        })
  ...
  @computed public get thumbnailURL() {
    if (this.tempValue) {
  ...
  @action public async onSave(isNew: boolean) {
