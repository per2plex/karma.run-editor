...
function renderOptionEntry(optionEntry: Select.OptionEntry) {
  return (
    <option key={optionEntry.key} value={optionEntry.key} disabled={optionEntry.disabled}>
      {'\u00a0\u00a0'.repeat(optionEntry.depth || 0)}{optionEntry.label}
    </option>
  )
}
...
export class Select extends React.Component<Select.Props> {
  ...
  public render() {
    ...
    switch (this.props.type) {
      default:
      case SelectType.Light: style = Select.LightStyle; break
      case SelectType.Transparent: style = Select.TransparentStyle; break
      case SelectType.Test: style = Select.TestStyle; break
    }

    return (
      <select className={`${SelectStyle} ${style}`}
        ...
        {this.props.options.map(option => (
          isOptionGroup(option) ? renderOptionGroup(option) : renderOptionEntry(option)
        ))}
      </select>
    )
  }
