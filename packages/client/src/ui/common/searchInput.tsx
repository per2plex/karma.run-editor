...
export class SearchInput extends React.Component<SearchInput.Props, SearchInput.State> {
  ...
  private handleFocus = () => {this.setState({hasFocus: true})}
  private handleBlur = () => {this.setState({hasFocus: false})}
      ...
      resultElements = (
        <div className={`${SearchInputStyle}_resultContainer`}>
          {results.map((result, index) => (
            <SearchInputItem key={result.id}
              item={result} selected={index === this.state.selectedIndex}
    ...
    return (
      <div className={SearchInputStyle} onKeyDown={this.handleKeyDown}>
        <div className={`${SearchInputStyle}_input`}>
          <TextInput {...inputProps} onFocus={this.handleFocus} onBlur={this.handleBlur} type={TextInputType.Dark} />
        </div>
        {resultElements}
      </div>
    )
...
export class SearchInputItem extends React.Component<SearchInputItem.Props> {
  ...
  public render() {
    return (
      <a key={this.props.item.id} className={SearchInputItemStyle}
        href={this.props.item.href} data-selected={boolAttr(this.props.selected)}
