...
import { Select, ButtonType, SelectType } from '../common'
...
export type Control = 'bold' | 'italic' | 'underline' | 'strikethrough' | 'orderedList' | 'unorderedList'
...
const CustomImmutableBlock: React.StatelessComponent<CustomImmutableBlock.Props> = (props) => {
  return (
    <div style={{...props.blockProps.style, userSelect: 'none'}}>
...
function iconForControl(control: Control) {
  switch (control) {
    case 'bold': return IconName.FormatBold
    case 'italic': return IconName.FormatItalic
    case 'strikethrough': return IconName.FormatStrikethrough
    case 'underline': return IconName.FormatUnderline
    case 'unorderedList': return IconName.FormatUnorderedList
    case 'orderedList': return IconName.FormatOrderedList
...
const LinkEntityRenderer: React.StatelessComponent = (props) => {
  return (
    <a className={LinkEntityRendererStyle}>
      {props.children}
    </a>
  )
}
...
const UnknownEntityRenderer: React.StatelessComponent = (props) => {
  return (
    <a className={UnknownEntityRendererStyle}>
      {props.children}
    </a>
  )
}
...
export class RichTextInput extends React.Component<RichTextInput.Props, RichTextInput.State> {
  ...
  private lengthOfSelection(editorState: EditorState) {
    ...
    if (startKey === endKey) {
      ...
      let totalLength = (startBlock.getLength() - selection.getStartOffset()) + selection.getEndOffset() + 1
  ...
  private handlePastedText = (text: string, _html: string, editorState: EditorState): DraftHandleValue => {
    if (this.props.maxLength != undefined) {
      const contentLength = editorState.getCurrentContent().getPlainText().length
      const selectionLength = this.lengthOfSelection(editorState)
  ...
  private handleControlClick = (control?: number | string) => {
    switch (control as Control) {
      case 'unorderedList': return this.toggleBlockStyle('unordered-list-item')
      case 'orderedList': return this.toggleBlockStyle('ordered-list-item')
      case 'underline': return this.toggleInlineStyleToSelection('UNDERLINE')
      case 'strikethrough': return this.toggleInlineStyleToSelection('STRIKETHROUGH')
      case 'italic': return this.toggleInlineStyleToSelection('ITALIC')
      case 'bold': return this.toggleInlineStyleToSelection('BOLD')
    }
  }
  ...
  private isControlActive(control: Control) {
    ...
    switch (control) {
      case 'bold': return currentStyle.has('BOLD')
      case 'italic': return currentStyle.has('ITALIC')
      case 'underline': return currentStyle.has('UNDERLINE')
      case 'strikethrough': return currentStyle.has('STRIKETHROUGH')
      case 'orderedList': return blockType === 'ordered-list-item'
      case 'unorderedList': return blockType === 'unordered-list-item'
    }
  ...
  private removeEntity() {
    ...
    contentState = Modifier.applyEntity(
      contentState, selectionstate, null as any
    )
  ...
  private handleInlineStyle = (inlineStyles: DraftInlineStyle) => {
    ...
    const styles = this.props.styleGroups.reduce((prev, styleGroup) => {
    ...
    return activeStyles.reduce((prev, style) => {
      return Object.assign(prev, style.style)
    }, {} as object)
  }

  private handleBlockStyle = (contentBlock: ContentBlock) => {
    const block = this.props.blocks && this.props.blocks.find(
      block => block.type === contentBlock.getType()
    )

    const element = this.props.elements && this.props.elements.find(
      element => element.type === contentBlock.getType()
    )
    ...
    return {component: CustomBlock, props: {style: {
      fontSize: '1em',
  ...
  private handleLinkClick = () => {
    ...
    this.props.onOpenLinkEditor(data, (data) => {
  ...
  private buttonForControl(control: Control) {
    return (
      <Button key={control}
        ...
        type={ButtonType.Icon} data={control}
  ...
  private handleBlockChange = (newBlock: string | undefined) => {
    ...
    if (blockSpec.dataKey) {
      ...
      this.props.onOpenBlockEditor(blockSpec.dataKey, data, (data) => {
  ...
  private handleInsertElement = (type?: string | number) => {
    ...
    return this.props.onOpenBlockEditor(element.dataKey, undefined, (data) => {
  ...
  private handleStyleChange = (newStyle: string | undefined, group?: string | number) => {
    ...
    if (selection.isCollapsed()) {
      let currentOverrideStyle = editorState.getInlineStyleOverride() || Immutable.OrderedSet<string>()

      currentOverrideStyle = currentOverrideStyle.filter(
        activeStyle => !styleGroup.styles.some(
          style => activeStyle === style.type
        )
      ...
      setTimeout(() => {
        this.props.onChange(EditorState.setInlineStyleOverride(
          this.props.value, currentOverrideStyle
        ))
      }, 0)
    ...
    if (styleGroup) {
      styleGroup.styles.forEach(style => {
        newContent = Modifier.removeInlineStyle(
          newContent, selection, style.type
        )
      })
    }
  ...
  public render() {
    ...
    if (this.props.styleGroups) {
      ...
      this.props.styleGroups.forEach((styleGroup) => {
        const options = styleGroup.styles.map(style => ({
          key: style.type, label: style.label || style.type
        }))

        const activeStyle = styleGroup.styles.find(style => activeStyles.has(style.type))

        if (buttons.length > 0) buttons.push(<ControlSeparator key={`separator_${styleGroup.name}`} />)

        buttons.push(
          <FlexList key={styleGroup.name}>
            {styleGroup.label && <label>{styleGroup.label}</label>}
            <FlexItem>
              <Select type={SelectType.Transparent}
                options={options}
                id={styleGroup.name}
                unselectedLabel={'----'}
    ...
    if (this.props.blocks) {
      ...
      const options = this.props.blocks.map(block => ({
        key: block.type, label: block.label || block.type
      }))
    ...
    if (this.props.links) {
      ...
      buttons.push(
        <Button key='insertLink' selected={this.selectionHasLink()}
          type={ButtonType.Icon} icon={IconName.InsertLink}
          onMouseDown={this.handleLinkClick} />
      )

      buttons.push(
        <Button key='removeLink'
          type={ButtonType.Icon} icon={IconName.RemoveLink}
