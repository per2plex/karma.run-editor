...
export class GraphView extends React.Component<GraphView.Props> {
  ...
  public async reload() {
    ...
    this.nodes = new DataSet<Node>(nodesForGraphMap(graphMap, this.props.editorStore.viewContextMap))
    ...
    this.network = new Network(this.element!, {nodes: this.nodes, edges}, {
      autoResize: true, groups,
...
function groupsForViewContexts(viewContexts: ViewContext[]) {
  return viewContexts.reduce((groups, viewContext) => {
    groups[viewContext.model] = {
      color: viewContext.color,
      font: {
        color: color(viewContext.color!).darken(0.1).toHexString(),
        face: FontFamily.primary,
        strokeWidth: 1,
        strokeColor: color(viewContext.color!).darken(0.6).toHexString()
      }
    }
