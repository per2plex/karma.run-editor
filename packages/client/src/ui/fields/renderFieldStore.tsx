...
export function renderValueStore(store: FieldStore, opts: RenderOpts): JSX.Element {
  if (store instanceof OptionalFieldStore) { // Optional
    return <OptionalFieldRenderer {...opts} store={store} />
  } else if (store instanceof FieldsetStore) { // Fieldset
    return <FieldsetRenderer {...opts} store={store} />
  } else if (store instanceof TextFieldStore) { // Text
    return <TextFieldRenderer {...opts} store={store} />
  } else if (store instanceof RefFieldStore) { // Ref
    return <RefFieldRenderer {...opts} store={store} />
  } else if (store instanceof CheckboxFieldStore) { // Checkbox
    return <CheckboxFieldRenderer {...opts} store={store} />
  } else if (store instanceof DateTimeFieldStore) { // DateTime
    return <DateTimeFieldRenderer {...opts} store={store} />
  } else if (store instanceof IntFieldStore || store instanceof FloatFieldStore) { // NumberField
    return <NumberFieldRenderer {...opts} store={store} />
  } else if (store instanceof ListFieldStore) { // ListField
    return <ListFieldRenderer {...opts} store={store} />
  } else if (store instanceof SelectFieldStore) { // SelectField
    return <SelectFieldRenderer {...opts} store={store} />
  } else if (store instanceof FileFieldStore) { // FileField
    return <FileFieldRenderer {...opts} store={store} />
  } else if (store instanceof RichtTextFieldStore) { // FileField
    return <RichTextFieldRenderer {...opts} store={store} />
  } else if (store instanceof PasswordFieldStore) { // PasswordField
    return <PasswordFieldRenderer {...opts} store={store} />
  } else if (store instanceof MapFieldStore) { // MapField
    return <MapFieldRenderer {...opts} store={store} />
  } else if (store instanceof EnumFieldStore) { // EnumField
    return <EnumFieldRenderer {...opts} store={store} />
  } else if (store instanceof TupleFieldStore) { // TupleField
    return <TupleFieldRenderer {...opts} store={store} />
  } else if (store instanceof OrFieldStore) { // OrField
    return <OrFieldRenderer {...opts} store={store} />
  } else if (store instanceof RecursionFieldStore) { // RecursionField
    return <RecursionFieldRenderer {...opts} store={store} />
  } else if (store instanceof ErrorFieldStore) { // ErrorField
