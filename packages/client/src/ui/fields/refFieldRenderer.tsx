...
import {
  Card, CardSection,
  CardError, Button, FlexList,
  DescriptionView, ButtonType, CardFooter
} from '../../ui/common'
...
@observer
export class RefFieldRenderer
  extends React.Component<RefFieldRenderer.Props, RefFieldRenderer.State> {
  ...
  private handleEditClick = () => {
    this.props.onEditEntry(this.props.store.model, this.props.store.id, (id: string | undefined) => {
      if (id) {
  ...
  public render() {
    ...
    if (this.state.error) {
      ...
      content = <CardSection><LoadingIndicator style={LoadingIndicatorStyle.Dark} /></CardSection>
      ...
      content = (
        <DescriptionView
          viewContext={viewContext} entry={entry}
