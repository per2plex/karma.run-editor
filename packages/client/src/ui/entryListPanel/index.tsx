    ...
    return (
      <div className={EntryToolbarFilterStyle}>
        ...
        {this.props.entryFilterStore.filterStores.length > 0 && (
          <Button icon={IconName.FilterList}
...
@observer
export class FilteredEntryListPanel<P = {}>
  extends React.Component<FilteredEntryListPanel.Props<P>, FilteredEntryListPanel.State> {
  ...
  public componentDidMount() {
    try {
      this.props.editorStore.loadEntriesForModel(this.props.viewContext.model)
    } catch (err) {
...
@observer
export class EntryListPanel extends FilteredEntryListPanel<EntryListPanel.Props> implements PanelComponent {
  private handleLocationTrigger = (location: AppLocation) => {
    switch (location.type) {
      case LocationType.EntryEdit: return this.props.onEntryEdit(this.props.viewContext, location.id)
      case LocationType.EntryDelete: return this.props.onEntryDelete(this.props.viewContext, location.id)
      case LocationType.EntryNew: return this.props.onNewEntry(this.props.viewContext)
    }
  }
  ...
  public render() {
    ...
    const newLocation = EntryNewLocation(this.props.viewContext.slug || this.props.viewContext.model)
    ...
    const toolbarFilter = (
      <EntryToolbarFilter viewContext={this.props.viewContext}
    ...
    if (this.state.filterStore!.isLoading) {
      ...
      content = (
        <EditEntryList viewContext={this.props.viewContext}
...
@observer
export class ChooseEntryListPanel extends FilteredEntryListPanel<ChooseEntryListPanel.Props> implements PanelComponent {
  ...
  public render() {
    ...
    if (this.state.filterStore!.isLoading) {
      ...
      content = (
        <ChooseEntryList viewContext={this.props.viewContext}
          entries={this.state.filterStore!.filteredEntries}
          reverseTags={this.props.editorStore.reverseTags}
