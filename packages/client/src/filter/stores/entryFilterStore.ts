...
export class EntryFilterStore {
  ...
  @observable.ref
  public selectedSortIndex: number = 0

  @observable.ref
  public shouldSortDescending: boolean = false
  ...
  @action
  private updateFilteredEntries = debounce(async () => {
    if (!this.entries) return

    let filter: Filter | undefined

    if (this.isFilterActive) {
      filter = CompositeFilter(this.filterStores.filter(store => store.filter != undefined).map(store => store.filter!))
