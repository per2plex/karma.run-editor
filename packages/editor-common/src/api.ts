...
export async function getAllEntriesForModel(id: string, session: Session): Promise<Entry[]> {
  ...
  while (true) {
    const slicedEntries = await rpcRequest(session, {
      mapList: {
        value: {
          slice: {
            value: {all: {model: id}},
            offset: {newInt: page * perPage},
            length: {newInt: perPage}
          }
        },
        expression: {metarialize: {id: {}}}
      }
    ...
    entries.push(...slicedEntries)

    if (slicedEntries.length < perPage) {
      break
    }
...
/** @throws {APIError} */
export async function getEntryForID(model: string, id: string, session: Session): Promise<Entry> {
  return await rpcRequest(session, {
...
/** @throws {APIError} */
export async function createEntry(model: string, value: any, session: Session): Promise<string> {
  return await rpcRequest(session, {
...
/** @throws {APIError} */
export async function deleteEntry(model: string, id: string, session: Session): Promise<string> {
  await rpcRequest(session, {
